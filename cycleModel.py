...
validation_data = CylcleDataset('./ValidationData.csv',seq_len=sequence_length, n_input_feat=num_input_feature*num_joint, n_output=num_joint)
...
test_data = CylcleDataset('./TestingData.csv',seq_len=sequence_length, n_input_feat=num_input_feature*num_joint, n_output=num_joint)
...
class PandaCycleNet(nn.Module):
    def __init__(self, device):
        ...
        if use_wandb is True:
            wandb.init(project="Panda Residual Cycle", tensorboard=False)
    ...
    def fit(self, trainloader, validationloader, print_every=1):
        ...
        for epoch in range(self.cur_epoch, self.cur_epoch + self.num_epochs):
            ...
            if use_wandb is True:
                ...
                # wandb_dict['Training Forward Loss'] = np.mean(forward_losses)
                wandb_dict['Training Backward Loss'] = np.mean(backward_losses)
                # wandb_dict['Training Forward Cycle Loss'] = np.mean(for_back_cycle_losses)
                # wandb_dict['Training Backward Cycle Loss'] = np.mean(back_for_cycle_losses)
...
PandaCycle.fit(trainloader=trainloader, validationloader=validationloader)
PandaCycle.eval()

PandaCycle.save_checkpoint()
for name, param in PandaCycle.state_dict().items():
    name= name.replace(".","_")
...
for conditions, states, inputs in testloader:
    ...
    if batch_idx == 0:
        traced_script_module = torch.jit.trace(PandaCycle.to('cpu'), [conditions.cpu(), states.cpu(), inputs.cpu()])
